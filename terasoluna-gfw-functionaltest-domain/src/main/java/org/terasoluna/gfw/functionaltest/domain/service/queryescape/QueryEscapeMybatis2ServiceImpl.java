...
import org.terasoluna.gfw.functionaltest.domain.repository.queryescape.TodoMybatis2Repository;

@Service("QueryEscapeMybatis2Service")
public class QueryEscapeMybatis2ServiceImpl implements QueryEscapeService {

	@Inject
	protected TodoMybatis2Repository queryEscapeMybatis2Repository;
	
	@Override
	public List<Todo> findAllByTitleLikePrefix(String title) {
		String condition = QueryEscapeUtils.toStartingWithCondition(title);
		return queryEscapeMybatis2Repository.findAllByTitleLike(condition);
	}
