...
<c:if test="${not empty page and page.totalElements != 0}">
	<h2 class="center-content">
		<span id="pagePosition">${f:h(page.number) + 1}</span> Page ( <span
			...
			id="rangeEnd">${(page.number * page.size) + page.numberOfElements}</span>
		/ <span id="totalResults">${page.totalElements}</span> results )
	</h2>
	<h3 class="center-content">Combination of queryTmpl And
		criteriaQuery (f:query)</h3>
	<div id="paginationCombinationOfQueryTmplAndCriteriaQueryAndFQuery"
		class="pagination">
	...
	<h3 class="center-content">Combination of queryTmpl And
		criteriaQuery (f:u)</h3>
	<div id="paginationCombinationOfQueryTmplAndCriteriaQueryAndFU"
		class="pagination">
		<t:pagination page="${page}"
			queryTmpl="page={page}&size={size}&sort={sortOrderProperty},{sortOrderDirection}"
